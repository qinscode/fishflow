  ...
  const filteredEquipmentSets = React.useMemo(() => {
    switch (selectedFilter) {
      case 'favorites':
        ...
        return equipmentSets.filter(set => set.tags.includes('收藏') || set.tags.includes('常用'));
      case 'recent':
              ...
              {filters.map((filter) => (
                ...
                >
                  <IconSymbol
                    ...
                    size={16}
                    color={selectedFilter === filter.key ? 'white' : theme.colors.text}
                  />
                  <ThemedText
                    type="bodySmall"
                    style={{
        ...
        {filteredEquipmentSets.length === 0 ? (
          ...
          <View style={styles.equipmentList}>
            {filteredEquipmentSets.map((set, index) => {
              ...
              return (
                <SlideInView key={set.id} direction="up" delay={400 + index * 100}>
                  <EquipmentSetCard
                    set={set}
                    stats={stats}
...
function EquipmentSetCard({ set, stats, onPress, onDelete, theme }: EquipmentSetCardProps) {
  return (
    <ThemedView type="card" style={[styles.equipmentCard, theme.shadows.md]}>
      <Pressable style={styles.cardPressable} onPress={onPress}>
        ...
        <View style={styles.equipmentDetails}>
          ...
          <View style={styles.equipmentRow}>
            <IconSymbol name="fish" size={16} color={theme.colors.textSecondary} />
            <ThemedText type="bodySmall" style={{ color: theme.colors.textSecondary }}>
              {set.bait}
            </ThemedText>
          </View>
          ...
          <View style={styles.waterTypesContainer}>
            <View style={styles.waterTypes}>
              {set.waterTypes.slice(0, 3).map((waterType) => (
                <View key={waterType} style={[styles.waterTypeTag, { backgroundColor: theme.colors.surface }]}>
              ...
              {set.waterTypes.length > 3 && (
                <View style={[styles.waterTypeTag, { backgroundColor: theme.colors.surface }]}>
                  <ThemedText type="caption" style={{ color: theme.colors.text }}>
                    +{set.waterTypes.length - 3}
                  </ThemedText>
                </View>
        ...
        {stats && (
          <View style={styles.statsRow}>
            ...
            <View style={styles.statItem}>
              <ThemedText type="bodySmall" style={{ color: theme.colors.secondary, fontWeight: '600' }}>
            ...
            <View style={styles.statItem}>
              <ThemedText type="bodySmall" style={{ color: theme.colors.accent, fontWeight: '600' }}>
                {stats.catchCount}
              </ThemedText>
              <ThemedText type="caption" style={{ color: theme.colors.textSecondary }}>
